...
public class EditGoalServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request,
        ...
        out.println("<title>Edit Goal - StudySync</title>");
        ...
        out.println("<h1>Edit Goal</h1>");

        out.println("<p class='subtitle'>Update your goal details and keep your progress on track.</p>");

        out.println("<form action='edit-goal' method='post'>");
        ...
        String pendingSelected =
                "PENDING".equalsIgnoreCase(goal.getStatus())
                ? "selected" : "";

        String progressSelected =
                "IN_PROGRESS".equalsIgnoreCase(goal.getStatus())
                ? "selected" : "";
        ...
        out.println("<option value='PENDING' "
                + pendingSelected + ">PENDING</option>");

        out.println("<option value='IN_PROGRESS' "
                + progressSelected + ">IN PROGRESS</option>");

        out.println("<option value='COMPLETED' "
                + completedSelected + ">COMPLETED</option>");
        ...
        out.println("<a href='goals' class='cancel-btn'>"
                + "Cancel</a>");
    ...
    protected void doPost(HttpServletRequest request,
        ...
        Goal oldGoal = goalDAO.getGoal(id);
        ...
        // Make sure logged-in user owns the goal
        if (oldGoal.getUserId() != user.getId()) {
            response.sendRedirect("goals");
            return;
        }
        ...
        updatedGoal.setUserId(user.getId());
        updatedGoal.setTitle(title);
        updatedGoal.setDescription(description);
        updatedGoal.setTargetDate(targetDate);
        updatedGoal.setStatus(status);
    ...
    // Helper method to safely display text in HTML
